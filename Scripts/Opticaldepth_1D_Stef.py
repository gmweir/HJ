...
def frequencybw(freqc,numpts=1,freqbw=None): #Create multiple points around central value as finite bandwith of ECE
    if freqbw is None:
        freqbw = 0.2*_np.ones_like(freqc) #200 MHz spread
...
datafolder = _os.path.abspath(_os.path.join('..','..','..','..', 'SyntheticDiagnostic','Opticaldepth','RayTraceTwoITB'))#location of MCviewer Ray Trace output

single=True #switches between plotting tau and Im(k) for single value of numpts or all values up to numpts
...
freqc=(62,64,66,68,70) #central frequencies of the ECE
...
qparab_parameters_NITB=[0.051783,0.002,10.208,3.4433,0.72003,0.5] #excel [t0,g,p,q,h,w]
...
_plt.figure()
_plt.plot(roa,Tprofile[0],label='T')
_plt.plot(roa,TITBprofile[0],label='ITB T')
_plt.plot(rho,T,label='MCViewer T')
_plt.xlabel(r'$\rho$') 
_plt.ylabel('T [keV]')
...
_plt.figure()
_plt.plot(roa,Nprofile[0],label='n')
_plt.plot(roa,NITBprofile[0],label='ITB n')
_plt.xlabel(r'$\rho$') 
_plt.ylabel('n [10^19 parts/m^3]')
_plt.title('Density profiles qparab along plasma roa')

if single:
    tau=_np.zeros((len(freqc)))
    kImk=_np.zeros((len(freqc),len(rho)))
    freq=frequencybw(freqc,numpts) #calculate used frequency points
    [tau[:],info]=opticaldepth_X2_1D(Te=_np.array(Tprofile[0]), 
