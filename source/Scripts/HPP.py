...
class HPP(_ut.Struct):
    
    cmPerGHz = 1

    def datasource(self):
        if self.machine == 'HJ':
            self.datafolder = _os.path.abspath(_os.path.join('..','bin'))
        # endif
        if self.verbose:
    ...
    def getECErf(self):
        if self.machine == 'W7X':
            self._getW7Xconfig(self, date)            
        elif self.machine == 'HJ':
            self._getHJconfig(self, date)
        elif self.machine == 'HSX': 
            self._getHSXconfig(self, date)            
        # endif
            
    
    def _getHJconfig(self, igch=None):
        self.ch = _np.asarray((_np.linspace(0,15,16)), dtype=_np.int64)
        if igch is None:
            igch = [1,7,14,15,16]
            igch = _np.asarray(igch, dtype=_np.int64)
        # endif
        self.msk = _np.ones(_np.shape(self.ch), dtype=_np.bool)
        self.msk[self.igch-1] = False
        self.ch = self.ch[self.msk]

        self.LO = _np.asarray(56, dtype=_np.float64)     # [GHz]
        self.IF = [2.5, 4.5, 6.5, 8.5, 10.5, 12.5, 14.5, 16.5, 
                   3.5, 5.5, 7.5, 9.5, 11.5, 13.5, 15.5, 17.5]
        self.IF = _np.asarray(self.IF, dtype=_np.float64)
        self.ece_rf = self.LO+self.IF
        self.ece_rf = self.ece_rf[self.msk]
    # end def _getHJconfig
    
    
    def _getHSXconfig(self, igch=None):
        self.ch = _np.asarray((_np.linspace(0,15,16)), dtype=_np.int64)
        if igch is None:
            igch = [1,7,14,15,16]
            igch = _np.asarray(igch, dtype=_np.int64)
